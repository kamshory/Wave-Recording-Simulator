...
function calcSamplingRate() {
    var maximumFrequency = document.querySelector('#maximum-frequency').value;
    var samplingRate = maximumFrequency * ratio;
...
function calculateFile() {
    ...
    bitDepth = document.querySelector('#bit-depth').value;
...
function drawAll() {
    ...
    var noise1 = createSignal(amplitude2/2, period2, phase2, resolution2);
    var noise2 = createSignal(amplitude2/2, period3, phase3, resolution3);
    var signal2 = combineSignal(noise1, noise2);
    ...
    var sampled = sampling(original, resolution1, ratio, bitDepth);
 ...
 * @param {number} lBitDepth Bit depth
 */
function sampling(signal, lResolution, lRatio, lBitDepth) {
    var result = [];
    var max = signal.length;
    var step = Math.round(lResolution / lRatio);
    for (var i = 0; i <= max; i += step) {
        if (typeof signal[i] != 'undefined') {
            result.push(quantizating(signal[i], lBitDepth));
        }
    }
    return result;
}
function quantizating(level, lbitDepth)
{
    var levelY = JSON.parse(JSON.stringify(level));
    var val;
    var val2;
    var step = 1;
    if(lbitDepth == '3')
    {
        step = Math.round(stepY / 3);
        val = levelY.y * stepY;
        val2 = Math.round(val / step) * (step/stepY);
        levelY.y = val2;
    }
    else if(lbitDepth == '4')
    {
        step = Math.round(stepY / 7);
        val = levelY.y * stepY;
        val2 = Math.round(val / step) * (step/stepY);
        levelY.y = val2;
    }
